...
void handle_advanced_repeat(bool is_pressed, uint16_t pressed_time,
  ...
  if (!is_pressed) return;  // キーが押されていない
  if (!*code_sent) return;              // コードがまだ送信されていない
  if (*active_flag) return;             // 既にアクティブ
  if (!*rapid_press) return;            // 連続タップされていない
  if (layer_state_is(_ADJUST)) return;  // ADJUSTレイヤーがアクティブ
...
bool handle_tap_key(bool pressed, uint16_t pressed_time,
                    uint16_t *released_time, bool *code_sent, bool *rapid_press, bool *repeat_active,
                    bool *mod_key_pressed, bool other_pressed, uint16_t keycode,
                    uint16_t record_time) {
  if (pressed) {
    return false;
  }

  // コードが既に送信済みなら何もしない（2重タップ防止）
  if (*code_sent) {
  ...
  // 離した時の単打判定
  if (timer_elapsed(pressed_time) < TAPPING_TERM && !other_pressed && !*rapid_press) {
...
void apply_active_mods(uint16_t keycode) {
  if (henkan_state.pressed) {
    register_os_specific_key(KC_LWIN);
  }
  if (mhenkan_state.pressed) {
    register_os_specific_key(KC_LALT);
  }
  if (c_bspc_state.pressed) {
    register_os_specific_key(KC_LCTL);
  }
  if (c_spc_state.pressed) {
...
bool handle_lower_key(keyrecord_t *record) {
  ...
  bool result = handle_tap_key(
      ...
      &lower_state.code_sent, &lower_state.rapid_press, &lower_state.repeat_active,
      &lower_state.mods_active, lower_state.other_key_pressed, KC_BSPC,
      record->event.time);
...
bool handle_raise_key(keyrecord_t *record) {
  if (record->event.pressed) {
    if (timer_elapsed(raise_state.released_time) < TAPPING_TERM &&
        !raise_state.code_sent && !raise_state.other_key_pressed) {
  ...
  bool result =
      handle_tap_key(false, raise_state.pressed_time,
                     &raise_state.released_time, &raise_state.code_sent,
                     &raise_state.rapid_press, &raise_state.repeat_active, &raise_state.mods_active,
                     raise_state.other_key_pressed, KC_SPC, record->event.time);
...
bool handle_henkan_key(keyrecord_t *record) {
  if (record->event.pressed) {
    ...
    bool result = handle_tap_key(
        ...
        &henkan_state.code_sent, &henkan_state.rapid_press, &henkan_state.repeat_active,
        &henkan_state.mods_active, henkan_state.other_key_pressed, KC_HENKAN,
        record->event.time);
...
bool handle_mhenkan_key(keyrecord_t *record) {
  if (record->event.pressed) {
    ...
    bool result = handle_tap_key(
        ...
        &mhenkan_state.code_sent, &mhenkan_state.rapid_press, &mhenkan_state.repeat_active,
        &mhenkan_state.mods_active, mhenkan_state.other_key_pressed, KC_MHENKAN,
        record->event.time);
...
bool handle_c_bspc_key(keyrecord_t *record) {
  if (record->event.pressed) {
    ...
    bool result = handle_tap_key(
        ...
        &c_bspc_state.code_sent, &c_bspc_state.rapid_press, &c_bspc_state.repeat_active, &c_bspc_state.mods_active,
        c_bspc_state.other_key_pressed, KC_BSPC, record->event.time);
...
bool handle_c_spc_key(keyrecord_t *record) {
  if (record->event.pressed) {
    ...
    bool result = handle_tap_key(
        ...
        &c_spc_state.code_sent, &c_spc_state.rapid_press, &c_spc_state.repeat_active, &c_spc_state.mods_active,
        c_spc_state.other_key_pressed, KC_SPC, record->event.time);
